...
const Dashboard: React.FC<DashboardProps> = ({ state, t }) => {
  ...
  const stats = [
    ...
    { label: 'Activités totales', value: state.activities.length, color: 'text-blue-600', icon: '🗓️', bg: 'bg-blue-50 dark:bg-blue-900/10' }
  ];
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-4xl font-black text-slate-800 dark:text-white tracking-tighter">Tableau de Bord</h2>
          <p className="text-slate-500 dark:text-slate-400 font-medium">Synthèse globale de la plantation en temps réel.</p>
        </div>
        <div className="flex items-center space-x-3 bg-white dark:bg-slate-800 px-4 py-2 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
           ...
           <span className="text-xs font-black uppercase tracking-widest text-slate-400">Système Connecté</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-xl font-black text-slate-800 dark:text-white">Flux des Ventes</h3>
            <select className="bg-slate-50 dark:bg-slate-700 border-none rounded-xl text-xs font-bold px-3 py-1.5 outline-none">
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col">
          <h3 className="text-xl font-black text-slate-800 dark:text-white mb-6">Activités Récentes</h3>
          <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-2">
            {state.activities.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-slate-400 py-10 opacity-50">
                <span className="text-4xl mb-2">📭</span>
                <p className="text-sm font-bold italic">Rien à signaler</p>
              </div>
            ) : (
