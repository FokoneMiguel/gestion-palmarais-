...
const Login: React.FC<LoginProps> = ({ 
  onLogin, users, t, theme, language, onThemeToggle, onLanguageToggle, addToast 
}) => {
  ...
  return (
    <div className="w-full min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 px-4 transition-colors">
      <div className="absolute top-6 right-6 flex space-x-3">
        ...
        <button onClick={onThemeToggle} className="p-3 text-xl bg-white/50 dark:bg-slate-800/50 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">{theme === 'light' ? '🌙' : '☀️'}</button>
      </div>
